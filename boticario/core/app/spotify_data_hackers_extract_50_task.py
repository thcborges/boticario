...
class SpotifyDataHackersExtract50Task(Task):
    __s3_path = 'spotify_data_hackers_50'

    def __get_api_data(self) -> DataFrame:
        logging.info('Getting data from spotify API')
        data_frame = SpotifyDao.get_first_50_records_to_data_hackers_search()
    ...
    def __load_data(self, data_frame: DataFrame):
        ...
        s3_key = (
            f'{self.__s3_path}/spotify_data_hackers_50_{timestamp}.parquet'
        )
        SpotifyDlo.copy_data_frame_to_spotify_data_hackers_50(data_frame, s3_key)
